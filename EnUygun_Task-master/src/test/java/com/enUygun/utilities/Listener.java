...
public class Listener extends TestListenerAdapter
{
	...
	public void onStart(ITestContext testContext)
	{
		...
		htmlReporter.config().setDocumentTitle("UI En uygun Automation Report"); // Tile of report
		htmlReporter.config().setReportName("UI En uygun Testing Report"); // name of the report
		htmlReporter.config().setTheme(Theme.STANDARD);
		
		extent=new ExtentReports();
		extent.attachReporter(htmlReporter);
		extent.setSystemInfo("Project Name","En uygun Automation");
		extent.setSystemInfo("Host name","11.111.111.111");
		extent.setSystemInfo("Environment","Test");
		extent.setSystemInfo("user","Mert Sevtekin");
